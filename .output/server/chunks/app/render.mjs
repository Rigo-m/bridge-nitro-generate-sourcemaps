...
for (const type of ["private", "public"]) {
  for (const key in _runtimeConfig[type]) {
    _runtimeConfig[type][key] = destr__default(process.env[key] || _runtimeConfig[type][key]);
  }
}
const privateConfig = deepFreeze(defu__default(_runtimeConfig.private, _runtimeConfig.public));
